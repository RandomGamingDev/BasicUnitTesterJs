const test = new UnitTester("Root Test", "The test that starts it all! (it also stores all the results and can be parsed into a JSON for storage of said results)",
  ...
  [
    new UnitTester("This subtest is mean to fail if uncommented!", "Something nice about this unit tester is that it keeps on testing even after something fails, whcih means that you'll be able to check everything else in case you're having trouble with a specific problem which especially helps if you can find other issues happening at the same time since the issues could be causing one another or correlated! (Note: Although tests following will still run, tests that are children of this test won't)",
      () => {
...
if (test.failed_sub_tests.length == 0)
  console.log("The unit test executed successfully");
else {
